...
{
    pr_debug("%s enter\n", __func__);
    aw8695->play_mode = AW8695_HAPTIC_STANDBY_MODE;//add 20190409
...
static unsigned char aw8695_haptic_get_dbgstat(struct aw8695 *aw8695)
{
    unsigned char ret = 0;
    unsigned char reg_val = 0;
...
static int aw8695_rtp_trim_lra_calibration(struct aw8695 *aw8695)
{
    ...
    if(fre_val == 3) {
        theory_time = (aw8695->rtp_len / 12000) * 1000000; /*12K */
    } else if(fre_val == 2) {
        theory_time = (aw8695->rtp_len / 24000) * 1000000; /*24K */
    } else if(fre_val == 1 || fre_val == 0) {
        theory_time = (aw8695->rtp_len / 48000) * 1000000; /*48K */
    }

    if(theory_time < aw8695->microsecond) {
        /* diff over 2%, test data error */
        if((aw8695->microsecond - theory_time) > (theory_time*cali_err_thr/1000)) {
            return 0;
        }
        /* diff less 0.1%, no need to cali */
        if((aw8695->microsecond - theory_time) > (theory_time*cali_min_thr/1000)) {
            return 0;
        }
        real_code = ((aw8695->microsecond - theory_time)* 4000) / theory_time;
        real_code = ((real_code%10 < 5)? 0 : 1) + real_code/10;
        real_code = 32 + real_code;
    }
    if(theory_time > aw8695->microsecond) {
        /* diff over 2%, test data error */
        if((theory_time - aw8695->microsecond) > (theory_time*cali_err_thr/1000)) {
            return 0;
        }
        /* diff less 0.1%, no need to cali */
        if((theory_time - aw8695->microsecond) > (theory_time*cali_min_thr/1000)) {
            return 0;
        }
        real_code = ((theory_time - aw8695->microsecond)* 4000) / theory_time ;
        real_code = ((real_code%10 < 5)? 0 : 1) + real_code/10;
        real_code = 32 - real_code;
    }
    pr_debug("%s microsecond=%ld, theory_time=%d, real_code=%d\n",
            __func__, aw8695->microsecond, theory_time, real_code);
...
static int aw8695_rtp_osc_calibration(struct aw8695 *aw8695)
{
    ...
    while(1) {
        ...
        reg_dbgstat = aw8695_haptic_get_dbgstat(aw8695);
        if(reg_dbgstat & AW8695_BIT_DBGSTAT_FF_EMPTY) {
...
static int aw8695_haptic_audio_init(struct aw8695 *aw8695)
{
    ...
    aw8695_i2c_read(aw8695, AW8695_REG_DATDBG, &reg_val);
...
static void aw8695_haptic_audio_work_routine(struct work_struct *work)
{
    ...
    memcpy(&aw8695->haptic_audio.ctr,&aw8695->haptic_audio.data[aw8695->haptic_audio.cnt],
	...
	if(AW8695_HAPTIC_CMD_ENABLE == aw8695->haptic_audio.ctr.cmd) {
	    if(AW8695_HAPTIC_PLAY_ENABLE == aw8695->haptic_audio.ctr.play) {
		    do_gettimeofday(&aw8695->audio_trig);
		    //vincent surport touch 
		    aw8695->touch_audio_gap = (aw8695->audio_trig.tv_sec - aw8695->touch_press.tv_sec)*1000000 +
			    (aw8695->audio_trig.tv_usec - aw8695->touch_press.tv_usec);
		    pr_info("%s: haptic_audio_play_start touch_audio_gap %ld \n", __func__,aw8695->touch_audio_gap);
		    if (((aw8695->touch_audio_gap > AW8695_HAPTIC_TOUCH_MIN_GAP)&&(aw8695->touch_audio_gap < AW8695_HAPTIC_TOUCH_MAX_GAP)&&(aw8695->touch == 1))||(aw8695->touch_enable == AW8695_HAPTIC_TOUCH_DISABLE))
		    {
...
static int aw8695_haptic_init(struct aw8695 *aw8695)
{
    ...
    mutex_lock(&aw8695->lock);
    //vincent surport touch 
    aw8695->touch_enable = AW8695_HAPTIC_TOUCH_DISABLE;
...
static ssize_t aw8695_touch_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8695_touch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	if (val == 1)//press
	{
		do_gettimeofday(&aw8695->touch_press);
	}
...
static ssize_t aw8695_touch_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8695_touch_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
	...
	val = val*2/3;
	printk(KERN_DEBUG "[%s]set vmax to (src*2/3)=%d.\n", __func__, val);
#endif
...
#ifdef CONFIG_ZTEMT_LED_BRIGHTNESS_CTL
	val = val*2/3;
	printk(KERN_DEBUG "[%s]set gain to (src*2/3)=%d.\n", __func__, val);
#endif
        ...
        const char *buf, size_t count)
{
    ...
    if(6 == sscanf(buf, "%d %d %d %d %d %d", &databuf[0], &databuf[1], &databuf[2],
        ...
        if(AW8695_HAPTIC_CMD_STOP == aw8695->haptic_audio.data[aw8695->haptic_audio.cnt].cmd) {
...
static DEVICE_ATTR(touch, S_IWUSR | S_IRUGO, aw8695_touch_show, aw8695_touch_store);
static DEVICE_ATTR(touch_enable, S_IWUSR | S_IRUGO, aw8695_touch_enable_show, aw8695_touch_enable_store);
//vincent surport touch 
