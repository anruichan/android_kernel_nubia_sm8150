...
#define SYNAP_PINCTRL_STATE_ACTIVE    "pmx_ts_active"
#define SYNAP_PINCTRL_STATE_SUSPEND   "pmx_ts_suspend"
#endif
...
static int nubia_synap_gamemode_enable(struct synaptics_rmi4_data *rmi4_data, bool on)
{
	...
	unsigned char reg_141 = 0x04;
	unsigned char reg_438 = 0x01;
	unsigned char reg_016[] = {0x01,0x01,0x26,0x26,0x80,0x80,0x00,0x00,0x05,0x2a,0x01,0x0c,0x28,0x04,0xff};
	unsigned char reg_138[] = {0x2e,0x00,0x07,0x02,0x00,0x22,0x1e};
	unsigned char reg_12a[] = {0x02,0x22,0x00,0x11,0x00,//1
				   0x00,0x00,0x03,0x00,0x01,//2
				   0x29,0x02,0x22,0x00,0x10,//3
				   0x00,0x0d,0x00,0x04,0x00,//4
				   0x01,0x26,0x02,0x22,0x00,//5
				   0x10,0x00,0x0e,0x00,0x09,//6
				   0x00,0x02,0x22,0x02,0x22,//7
				   0x00,0x0f,0x00,0x10,0x00,//8
				   0x09,0x00,0x02,0x23,0x85,//9
				   0x53,0x00,0x0f,0x00,0x00,//10
				   0x00,0x03,0x00,0x01,0x29,//11
				   0x85,0x48,0x00,0x0e,0x00,//12
				   0x0d,0x00,0x04,0x00,0x01,//13
				   0x26,0x85,0x48,0x00,0x0e,//14
				   0x00,0x0e,0x00,0x09,0x00,//15
				   0x02,0x22,0x85,0x48,//16
				   };

	if (on) {
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x012a, &reg_12a[0], 79);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0138, &reg_138[0], 7);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0016, &reg_016[0], 15);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0438, &reg_438, 1);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141, 1);

	} else {
		if(on_flag != on) 
		{
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x0022, &reset, sizeof(reset));
		} 
		else {
			return retval;
		}
...
{
	int ret = 0;
	pr_err("%s: enter synaptics_nubia_tptest_show \n", __func__);
...
#endif
	pr_info("%s: start update fw\n", __func__);
	synaptics_fw_updater(NULL);
	pr_info("%s: end update fw\n", __func__);
...
static int synaptics_ts_pinctrl_init(struct device *dev, struct synaptics_rmi4_data *bdata)
{
        ...
        /* active state */
        bdata->pin_sta_active = pinctrl_lookup_state(bdata->pinctrl,
                                SYNAP_PINCTRL_STATE_ACTIVE);
        if (IS_ERR_OR_NULL(bdata->pin_sta_active)) {
                r = PTR_ERR(bdata->pin_sta_active);
                pr_info("%s:Failed to get pinctrl state:%s, r:%d", __func__,
                                SYNAP_PINCTRL_STATE_ACTIVE, r);
                goto exit_pinctrl_put;
        }

        /* suspend state */
        bdata->pin_sta_suspend = pinctrl_lookup_state(bdata->pinctrl,
                                SYNAP_PINCTRL_STATE_SUSPEND);
        if (IS_ERR_OR_NULL(bdata->pin_sta_suspend)) {
                r = PTR_ERR(bdata->pin_sta_suspend);
                pr_info("%s:Failed to get pinctrl state:%s, r:%d", __func__,
                                SYNAP_PINCTRL_STATE_SUSPEND, r);
                goto exit_pinctrl_put;
        }
	...
	rmi4_data->fingers_on_2d = false;
        ...
        if (!retval && rmi4_data->pinctrl) {
                ...
                retval = pinctrl_select_state(rmi4_data->pinctrl,
                                        rmi4_data->pin_sta_active);
                if (retval < 0)
                        pr_info("%s:Failed to select active pinstate, r:%d", __func__, retval);
        }
#endif
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->enable_wakeup_gesture && g_power_flag) {
		disable_irq(bdata->irq_gpio);
		if (bdata->reset_gpio >= 0) {
			gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		}
